              ...
              <div class="p-6 text-gray-900">
                <x-flash-message class="text-center mb-8" />
                   @if (count($products) > 0)
                      ...
                      @endforeach
                      <div class="md:flex md:justify-end md:items-center gap-8">
                        <div>
                          小計:{{ number_format($totalPrice) }}円（税込）
                        </div>
                        <button class="text-white bg-indigo-500 border-0 py-2 px-6 md:mt-0 mt-4 focus:outline-none hover:bg-indigo-600 rounded">
                          <a href="{{ route('user.carts.checkout') }}">購入する</a>
                        </button>
                      </div>
                    @else
                        カートに商品がありません
